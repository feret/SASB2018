 ...
 * Last modification: Time-stamp: <2015-07-06 07:42:28 feret>
 * * 
 *  
...
let _,_,tmp =  
  disjoint_union uuu 
    (move_remanent_right_to 1. upu upu)
...
let _,_,tmp =  
  disjoint_union upu 
    (move_remanent_right_to 1. upp upp)
let reaction_du = 
  add_rule 1.1 0. ~directives:[Comment("$\kz$");Height 0.6] tmp 
let _,_,tmp =  
  disjoint_union ppu (move_remanent_right_to 1. ppp ppp)
let reaction_dp = 
  add_rule 1.1 0. ~directives:[Comment("$\kz$");Height 0.6] tmp 
let _,_,tmp =  
  disjoint_union upu (move_remanent_right_to 1. ppu ppu)

let reaction_gu = 
  add_rule 1.1 0. ~directives:[Comment("$\kx$");Height 0.6] tmp 
let _,_,tmp =  
  disjoint_union upp (move_remanent_right_to 1. ppp ppp)
let reaction_gp = 
  add_rule 1.1 0. ~directives:[Comment("$\kx$");Height 0.6] tmp 
...
let _,_,tmp =  
  disjoint_union upn
    (move_remanent_right_to 1. ppn ppn)
let reaction_gn = 
  add_rule 1.3 0. ~directives:[Comment("$\kx$");Height 0.6] tmp 
let _,_,tmp =  
  disjoint_union npu
    (move_remanent_right_to 1.5 npp npp)
let reaction_dn = 
  add_rule 1.3 0. ~directives:[Comment("$\kz$");Height 0.6] tmp 
