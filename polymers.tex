...
In this section, we explain how to refine the graph of the potential links of a given contact map, in order to take into the constraints about the reachable bio-molecular compounds.
These constraints may come from a static analysis \cite{SASB2016,KaSa} taken as a black box and they take the form of a set of patterns that shall occur in no reachable bio-molecular species.
...
For instance, the analysis that is described in \cite{SASB2016} can infer automatically, from the set of rules that describes the model and its set of initial state, that, in the early events of the integration of the epidermic growth factor, that the following patterns:
\begin{equation*}
...
\begin{theorem}
  \label{th:linkbis}
  Let $\graphsymb_{\textit{CM}}$ be a contact map.
  Let $P$ be a set o patterns.
  Let $G$ be a refinement of the graph of the potential links of the contact map, according to the set of patterns $P$.

If $G$ is acyclic, then the set $\{[E]_{\iso}\in \llbracket \graphsymb_{\textit{CM}} \rrbracket\;|\;E \text{ contains no occurrence of patterns belonging to the }$ is a finite set of bio-molecular compounds
(up to isomorphism) which are both compatib
Let $\agentfont{A}$ and $\agentfont{B}$ be two kinds of agent and
$i$ and $i'$ be two site names.

  We assume that there exists biomolecular compound
  $S$ that can be mapped into the $\Sigma$-graph $\graphsymb_{\textit{CM}}$ by the means of a morphism. We assume that no pattern in $P$ embeds into $S$.
  We assume that there exists a repeatable pattern $R$ that embeds into $S$.
  ...
  there exist two agents $n$ and $n'$ respectively of kinds $\agentfont{A}$
  and  $\agentfont{B}$, and a  cycle in the graph $G$ that passes by the vertice  $((n,i),(n',i'))$.
  Both following properties are equivalent:
  \begin{enumerate}
    \item There exists a repeatable pattern
    with an agent of kind $\agentfont{A}$ connected via its site $i$
