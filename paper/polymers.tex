...
The pattern that is made of three proteins, the first two bound via their respective sites \sitefont{a} and the last two bound via their respective sites \sitefont{b} (e.g.~see Fig.~\ref{fig:twoself:pattern}) is a repeatable pattern. Thus, an infinite number of bio-molecular compounds is compatible with the contact map.  }
\end{figure}
...
Firstly a pair of receptors \agentfont{EGFR} on the membrane of the cell shall be activated by the ligand \agentfont{EGF}. Once activated, they can form a dimer thanks to a symmetric bond via their respective sites
...
\centering\scalebox{0.6}{\includegraphics{generated_pictures/abc_pattern_b.pdf}}\smallskip\\
\end{minipage}}%

\subfigure[A chain with two extremities of the same kind but with the same bound site.]{%
\begin{minipage}{0.49\linewidth}
...
\centering\scalebox{0.6}{\includegraphics{generated_pictures/abc_pattern.pdf}}\bigskip\\
\end{minipage}}
...
\begin{defn}[iterations of a repeatable pattern]
...
\begin{figure}[t]
  ...
  {\begin{minipage}{\linewidth}
  \centering\scalebox{0.6}{\includegraphics{generated_pictures/abc_iteration_1.pdf}}
\end{minipage}}

  \subfigure[Second iteration.]%
  {\begin{minipage}{\linewidth}
  \centering\scalebox{0.6}{\includegraphics{generated_pictures/abc_iteration_2.pdf}}\end{minipage}}


  \subfigure[Third iteration.]%
  {\begin{minipage}{\linewidth}
  \centering\scalebox{0.6}{\includegraphics{generated_pictures/abc_iteration_3.pdf}}\end{minipage}}
...
Each site on a odd position and the next one always belong to the same kind of protein, whereas there always exists a link between each site on an even position and the next one. Due to this polarisation, it is tempting to consider the sub-sequence that is made of  each other site in that sequence of sites.
...
Each one corresponds to the triangle \agentfont{ABC} depending whether it is scanned clockwise or counter-clockwise. Further constraints would be required on the bio-molecular compounds of the model to prove that there is a finite amount of them (the contact map of the model is compatible with an infinite number of bio-molecular compounds).
\end{exmp}

\begin{exmp}[graph of the sites (EGF model)]
In Fig.~\ref{fig:egfr:gs}, we compute the graph of the sites for the contact map of the model of the early events in the integration of the epidermic growth factor. It is worth noticing that this graph has only one non trivial strongly connected component, which is depicted as follows:
...
In the following, we define a graph that stands for all the potential sequences of links that may occur consecutively on the repeatable patterns that are compatible with a given contact map. This graph is called the graph of the links.

\begin{defn}[graph of the links]
  Let $\graphsymb_{\textit{CM}}$ be a contact map.

  The contact map $\graphsymb_{\textit{CM}}$ is associated with a classical graph $(\mathcal{V},\mathcal{E})$, called the graph of the links that is defined as follows:
  \begin{itemize}
    \item $\mathcal{V}$ is the subset of the pairs of elements $(s,s')$ of the set  $\sites[\graphsymb_{\textit{CM}}]$ of the sites of the $\Sigma$-graph $\graphsymb_{\textit{CM}}$, such that $s'\in\links[\graphsymb_{\textit{CM}}](s)$.
    \item $\mathcal{E}$ is the subset of the pairs $((s,s'),(s'',s'''))$ of pairs of sites in $\mathcal{V}\times\mathcal{V}$ for which  there exists an agent $n\in\agents[\graphsymb_{\textit{CM}}]$ and two different site names $i$ and $i' \in\linksite(\type[\graphsymb_{\textit{CM}}](n))$ such that $s'=(n,i)$ and $s''=(n,i')$.
  \end{itemize}
  \end{defn}
...
\begin{figure}
  \subfigure[Contact map.]%
  {\begin{minipage}{0.3\linewidth}\label{fig:abc:gl:cm}\centering\scalebox{0.4}{\includegraphics{generated_pictures/abc_contact_map.pdf}}\end{minipage}}%
  \subfigure[Graph of the links.]%
  {\begin{minipage}{0.69\linewidth}\label{fig:abc:gl:gl}
  \xymatrix@C=0.cm@R=0.35cm{
...
\begin{exmp}[graph of the links  (ABC model)]
...
\begin{exmp}[graph of the links  (EGF model)]
In Fig.~\ref{fig:egfr:gl}, we compute the graph of the links  for the contact map of the model of the early events in the integration of the epidermic growth factor. It is worth noticing that this graph has only one non trivial strongly connected component:
\begin{equation*}\hspace*{1cm}\scalebox{1.5}{\xymatrix@C=0.2cm@R=0.35cm{\begin{minipage}{1.36cm}\scalebox{0.15}{\hspace*{1.5cm}\includegraphics{generated_pictures/egfr_egfr_c.pdf}}\end{minipage}
  \ar@{->}[rr]
  \ar@{->}@(lu,ld)
  &&
...
In this section, we explain how to refine the graph of the links  of a given contact map, in order to take into account some additional constraints about the bio-molecular compounds that are potentially reachable.
...
These constraints cannot be written in the contact map because the contact map describes only non relational information about the potential state of sites.
...
The analysis that is described in \cite{afp} generalises this approach to arbitrary cycles of proteins. In the example of the three kinds of protein that may form a triangle, this static analysis infers that no two \agentfont{A}s may occur in a reachable bio-molecular compound, by proving that the following pattern:
\begin{equation*}
\scalebox{0.6}{\includegraphics{generated_pictures/abc_pattern.pdf}}
\end{equation*}
is unreachable.
...
Firstly we label every edge of the graph of the links  by a chain of agents. More precisely, the label of an edge is obtained by fusing the second agent of the source node with the first agent of the target node.
We keep this orientation for the chain of agents that are now used to label the edges of the graph of the links (that is to say that two links are identified as respectively the source and the target of the chain of agents).
...
The edge is labelled with a chain of three agents. The source of the chain is the link between the site \sitefont{c} and the site \sitefont{n} whereas the target of the chain if the link between the two sites \sitefont{r}.
...
\begin{exmp}[graph refinement (ABC model)]
  \label{exmp:pruning}%
In the model with three kinds of protein that may form a triangle (\eg~see Fig.~\ref{fig:abc:gl:gl}), the node:
\begin{center}%
\scalebox{0.6}{\includegraphics{generated_pictures/link_b_c.pdf}}
\end{center}%
...
\end{center}%
\end{minipage}%
${\xrightarrow{\begin{minipage}{2.cm}\scalebox{0.3}{\includegraphics{generated_pictures/abc_pattern_bca.pdf}}\end{minipage}}}$%
\begin{minipage}{2.5cm}%
\begin{center}%
\scalebox{0.6}{\includegraphics{generated_pictures/link_c_a.pdf}}%
\end{center}%
\end{minipage}.
\end{center}
Thus, its incoming edge:
  ...
  \begin{minipage}{2.5cm}%
  \begin{center}%
  \scalebox{0.6}{\includegraphics{generated_pictures/link_a_b.pdf}}%
  \end{center}%
  \end{minipage}%
  ${\xrightarrow{\begin{minipage}{2cm}\scalebox{0.3}{\includegraphics{generated_pictures/abc_pattern_a.pdf}}\end{minipage}}}$%
  \begin{minipage}{2.5cm}%
  \begin{center}%
  \scalebox{0.6
  }{\includegraphics{generated_pictures/link_b_c.pdf}}%
  \end{center}%
  \end{minipage}
  \end{center}
may be safely replaced with the following one:
  ...
  \end{center}%
  \end{minipage}%
${\xrightarrow{\begin{minipage}{2.8cm}\scalebox{0.3}{\includegraphics{generated_pictures/abc_pattern.pdf}}\end{minipage}}}$%
\begin{minipage}{2.5cm}%
\begin{center}%
\scalebox{0.6}{\includegraphics{generated_pictures/link_c_a.pdf}}%
\end{center}%
\end{minipage}%
\end{center}
that is obtained as the composition of both edges.
This new edge may then be discarded since the pattern:
\begin{equation*}
\scalebox{0.6}{\includegraphics{generated_pictures/abc_pattern.pdf}}
\end{equation*}
is black-listed.
...
The same way, the edge:
  ...
  \begin{minipage}{2.5cm}%
  \begin{center}%
  \scalebox{0.6}{\includegraphics{generated_pictures/link_a_c.pdf}}%
  \end{center}%
  \end{minipage}%
  ${\xrightarrow{\begin{minipage}{2cm}\scalebox{0.3}{\includegraphics{generated_pictures/abc_pattern_acb.pdf}}\end{minipage}}}$%
  \begin{minipage}{2.5cm}%
  \begin{center}%
  \scalebox{0.6
  }{\includegraphics{generated_pictures/link_c_b.pdf}}%
  \end{center}%
  \end{minipage}
  \end{center}
may be safely replaced with the following one:
...
\end{center}%
\end{minipage}%
${\xrightarrow{\begin{minipage}{2.8cm}\scalebox{0.3}{\includegraphics{generated_pictures/abc_pattern_acba.pdf}}\end{minipage}}}$%
\begin{minipage}{2.5cm}%
\begin{center}%
\scalebox{0.6}{\includegraphics{generated_pictures/link_b_a.pdf}}%
\end{center}%
\end{minipage}%
\end{center}
which may then be discarded, since the pattern:
\begin{equation*}
\scalebox{0.6}{\includegraphics{generated_pictures/abc_pattern_acba.pdf}}
\end{equation*}
...
As a result, we obtain the refined graph of the links,
that is depicted in Fig.~\ref{fig:abc:gll:gl}.
\end{exmp}
...
\begin{figure}
  \subfigure[Contact map.]%
  {\begin{minipage}{0.3\linewidth}\label{fig:abc:gll:cm}\centering\scalebox{0.4}{\includegraphics{generated_pictures/abc_contact_map.pdf}}\end{minipage}}%
  \subfigure[Refinement of the graph of the links.]%
  {\begin{minipage}{0.69\linewidth}\label{fig:abc:gll:gl}
  \xymatrix@C=0.cm@R=0.35cm{
  \begin{minipage}{1.4cm}
  \scalebox{0.35}{\includegraphics{generated_pictures/link_a_b.pdf}}
  \end{minipage}
...
\ar@{->}[ll]
 ...
 In Fig.~\ref{fig:abc:gll:gl}, we refine the graph of the links  to take into account the constraints that two instances of \agentfont{A} may not occur in a same connected component. }
  \label{fig:abc:gll}
\end{figure}
...
In Fig.~\ref{fig:abc:gll}, we refine the graph of the links  for the contact map of the model \agentfont{ABC} by taking into account that any pattern with several instances of the protein \agentfont{A} is unreachable.
...
\begin{exmp}[refined graph of the links  (EGF model)]
