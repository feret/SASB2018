...
Each site on a odd position and the next one always belong to the same kind of proteins.
...
The label of an edge must be understood as an explanation about how the link of the source of this edge and of the target of this edge may be connected in  reachable bio-molecular compounds. When an edge is labelled with a chain that is unreacheable then this edge may be safely discarded.

The longer a chain of agents is, the more constraints it imposes. The second step consists in combining consecutive edges, in order to extend their labels into longer chains. It is worth noticing that given a node in the graph of the links, the target of the label of every incoming edge and the source of the label of every out-coming edge is indeed the pattern that is labelling the node.
We will keep this property as a structural invariant of the graph. This structural invariant will be preserved when combining the edges of the graph. Given a node in the graph of the links, an incoming edge and an out-coming edge may be composed to form an edge from the source of the incoming edge to the target of the out-coming edge. The label of this new edge is obtained by fusing the label of the out-coming edge and the label of the incoming edge by identifying the target of the former with the source of the later one. This way, we form a chain of agents from the source of the label of the incoming edge to the target of the label of the out-coming edge, which ensures that the structural invariant of the graph of the links is preserved.
